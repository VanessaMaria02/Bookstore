...
if(true){
    ...
    $sql = "UPDATE bestellungen SET b_anzahl = ?, b_timestamp = ? WHERE u_id = ? AND  b_timestamp = ? AND pr_id = ?";
    $statment = $db->prepare($sql);
    $statment->bind_param("isisi",$anzahl, $b_timestamp, $u_id, $b_timestamp, $pr_id);
    $statment->execute();
    $statment->close();
